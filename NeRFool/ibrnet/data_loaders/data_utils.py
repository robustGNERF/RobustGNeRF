...
def random_crop(rgb, camera, src_rgbs, src_cameras, size=(400, 600), center=None, src_depths=None):
    h, w = rgb.shape[:2]
    ...
    if out_w >= w or out_h >= h:
        if src_depths is None:
            ...
            return rgb, camera, src_rgbs, src_cameras, src_depths
    ...
    if src_depths is not None:
        src_depths = np.array(src_depths)
        src_depths = src_depths[:, center_h - out_h // 2:center_h + out_h // 2,
                center_w - out_w // 2:center_w + out_w // 2, :]
    ...
    if src_depths is None:
        return rgb_out, camera, src_rgbs, src_cameras, (center_h, center_w)
    else:
        return rgb_out, camera, src_rgbs, src_cameras, src_depths
...
def get_nearest_pose_ids(tar_pose, ref_poses, num_select, tar_id=-1, angular_dist_method='vector',
    ...
    if tar_id >= 0:
        assert tar_id < num_cams, 'tar_id={}, num_cams={}'.format(tar_id, num_cams)
