...
from ibrnet.render_ray import render_rays, render_rays_hybrid
...
def render_single_image(ray_sampler,
    ...
    for i in tqdm(range(0, N_rays, chunk_size)):
        chunk = OrderedDict()
        for k in ray_batch:
        ...
        if args is not None and (args.use_clean_color or args.use_clean_density):
            ...
            ret = render_rays_hybrid(chunk, model, featmaps,
                            projector=projector,
                            N_samples=N_samples,
    ...
    if 'mask' in all_ret['outputs_coarse'].keys():
