...
import os
import imageio
from nan.dataloaders.colmap_read_model import read_images_binary
...
def _load_data(basedir, factor=None, width=None, height=None, load_imgs=True, seanerf=False):
    ...
    imgfiles = sorted(imgdir.glob("*.[Jjp][Ppn][Gg]"))
    
    if poses.shape[-1] != len(imgfiles):
        imagesfile = basedir/'sparse'/'0'/'images.bin'
        ...
        print(f'{basedir}: Mismatch between imgs {len(imgfiles)} and poses {poses.shape[-1]} !!!!')
        return
...
if __name__ == '__main__':
    scene_path = Path('/home/qianqianwang/datasets/nerf_llff_data/trex/')
