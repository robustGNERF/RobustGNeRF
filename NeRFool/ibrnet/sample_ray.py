...
class RaySamplerSingleImage(object):
    def __init__(self, data, device, resize_factor=1, render_stride=1, load_gt_depth=False):
        super().__init__()
        ...
        if self.load_gt_depth:
            assert 'src_depths' in data.keys(), 'No src_depths found in data'
            self.src_depths = data['src_depths']   # [1, num_source_views, H, W]
            
            assert 'depth' in data.keys(), 'No depth found in data'
            self.depth_full = data['depth']
            
            if resize_factor == 1 and self.depth_full.shape[-1] != self.rgb.shape[-1]:  ## if the rgb is not scaled and depth is smaller than rgb, then scale depth
                self.depth_full = torch.nn.functional.upsample(self.depth_full.unsqueeze(0), scale_factor=(2, 2), mode='nearest').squeeze(0)
                self.src_depths = torch.nn.functional.upsample(self.src_depths, scale_factor=(2, 2), mode='nearest')

            self.depth = self.depth_full.reshape(-1)
        ...
        self.rays_o, self.rays_d, self.xy = self.get_rays_single_image(self.H, self.W, self.intrinsics, self.c2w_mat)
        if self.rgb is not None:
    ...
    def get_rays_single_image(self, H, W, intrinsics, c2w):
        ...
        rays_o = c2w[:, :3, 3].unsqueeze(1).repeat(1, rays_d.shape[0], 1).reshape(-1, 3)  # B x HW x 3, shape=[HW, 3]
    ...
    def get_all(self):
        ret = {'ray_o': self.rays_o.cuda(),
               ...
               'depth_full': self.depth_full.cuda() if self.depth_full is not None else None,
               'xyz': self.xy.cuda()
        }
        return ret
    ...
    def random_patch_sample(self, N_rand, patch_size):
        '''
        :param N_rand: number of rays to be casted
        :return:
        '''
        n_patches = N_rand // (patch_size**2)
        
        x0 = rng.randint(0, self.H - patch_size + 1, size=(n_patches, 1, 1))
        y0 = rng.randint(0, self.W - patch_size + 1, size=(n_patches, 1, 1))
        xy0 = np.concatenate([x0, y0], axis=-1)  # [n_patches, 1, 2]
        patch_idx = xy0 + np.stack(np.meshgrid(np.arange(patch_size), np.arange(patch_size), indexing='xy'),axis=-1).reshape(1, -1, 2)  # [n_patches, patch_size * patch_size, 2]

        select_inds = patch_idx[...,1] + self.W * patch_idx[...,0]  # [n_patches, patch_size * patch_size]
        # print('select_inds.shape:', select_inds.shape)
        
        select_inds = select_inds.reshape(-1)  # [n_patches * patch_size * patch_size]
        
        # print('H,W,stride:', self.H, self.W, self.render_stride)
