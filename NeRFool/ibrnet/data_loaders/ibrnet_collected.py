...
class IBRNetCollectedDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if self.mode == 'train':
            ...
            num_select = self.num_source_views #self.num_source_views + np.random.randint(low=-2, high=3)
        else:
            id_render = -1
            subsample_factor = 1
        ...
        if self.mode == 'train' and self.random_crop:
            crop_h, crop_w = 384 - 32, 512 - 32
            # print("IBRCollect",rgb.shape, src_rgbs.shape)
            rgb, camera, src_rgbs, src_cameras, center_coord = random_crop(rgb, camera, src_rgbs, src_cameras, size=(crop_h, crop_w))
        ...
        return {'rgb': torch.from_numpy(rgb[..., :3]),
                ...
                'center_coord' : center_coord if self.mode == 'train' else None
                }
