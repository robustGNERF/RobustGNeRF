...
def get_spline_poses(se3_start, se3_end, spline_num=7):
    n_pose = se3_start.shape[0]
    pose_nums = torch.arange(spline_num).reshape(1, -1).repeat(n_pose, 1)
    seg_pos_x = torch.arange(se3_start.shape[0]).reshape([n_pose, 1]).repeat(1, spline_num)

    se3_start = se3_start[seg_pos_x, :]
    se3_end = se3_end[seg_pos_x, :]
    spline_poses = SplineN_linear(se3_start, se3_end, pose_nums, spline_num)

    return spline_poses.reshape(n_pose, spline_num, 3, 4)


def SplineN_linear(start_pose, end_pose, poses_number, NUM, device=None):
    pose_time = poses_number.to(start_pose.device) / (NUM - 1)
