...
def raw2outputs(raw, z_vals, mask, white_bkgd=False, geo_noise=None):
    '''
    ...
    if geo_noise is not None and geo_noise > 0:
        sigma = sigma + torch.randn_like(sigma) * geo_noise  # add a gaussian noise with a std of geo_noise
    ...
    ret = OrderedDict([('rgb', rgb_map),
                       ('depth', depth_map),
                       ('weights', weights), # [N_rays, N_samples], used for importance sampling of fine samples
                       ('mask', mask),
...
def render_rays(ray_batch,
    ...
    rgb_feat, ray_diff, mask = projector.compute(pts, ray_batch['camera'],
                                                 ray_batch['src_rgbs'] if src_ray_batch is None else src_ray_batch['src_rgbs'],
                                                 ray_batch['src_cameras'] if src_ray_batch is None else src_ray_batch['src_cameras'],
    ...
    if 1:
        ...
        outputs_coarse = raw2outputs(raw_coarse, z_vals, pixel_mask,
                                    white_bkgd=white_bkgd, geo_noise=geo_noise)
...
def render_rays_hybrid(ray_batch,
                model,
                featmaps,
                projector,
                N_samples,
    ...
    # use clean features
    rgb_feat_clean, ray_diff_clean, mask_clean = projector.compute(pts, ray_batch['camera'],
                                                 ray_batch['src_rgbs'] if src_ray_batch is None else src_ray_batch['src_rgbs'],
                                                 ray_batch['src_cameras'] if src_ray_batch is None else src_ray_batch['src_cameras'],
                                                 featmaps=featmaps_clean[0])  # [N_rays, N_samples, N_views, x]
    pixel_mask_clean = mask_clean[..., 0].sum(dim=2) > 1   # [N_rays, N_samples], should at least have 2 observations
    raw_coarse_clean = model.net_coarse(rgb_feat_clean, ray_diff_clean, mask_clean)   # [N_rays, N_samples, 4]
    
    if args.use_clean_color:
        color = raw_coarse_clean[:, :, :3] # [N_rays, N_samples, 3]
    else:
        color = raw_coarse[:, :, :3]
        
    if args.use_clean_density:
        sigma = raw_coarse_clean[:, :, 3:4] # [N_rays, N_samples, 1]
    else:
        sigma = raw_coarse[:, :, 3:4]
    
    raw_coarse = torch.cat([color, sigma], dim=2)
    ...
    if N_importance > 0:
        ...
        # use clean features
        rgb_feat_sampled_clean, ray_diff_clean, mask_clean = projector.compute(pts, ray_batch['camera'],
                                                             ray_batch['src_rgbs'] if src_ray_batch is None else src_ray_batch['src_rgbs'],
                                                             ray_batch['src_cameras'] if src_ray_batch is None else src_ray_batch['src_cameras'],
                                                             featmaps=featmaps_clean[1])

        pixel_mask_clean = mask_clean[..., 0].sum(dim=2) > 1  # [N_rays, N_samples]. should at least have 2 observations
        raw_fine_clean = model.net_fine(rgb_feat_sampled_clean, ray_diff_clean, mask_clean)
        
        if args.use_clean_color:
            color = raw_fine_clean[:, :, :3] # [N_rays, N_samples, 3]
        else:
            color = raw_fine[:, :, :3]
            
        if args.use_clean_density:
            sigma = raw_fine_clean[:, :, 3:4] # [N_rays, N_samples, 1]
        else:
            sigma = raw_fine[:, :, 3:4]
        
        raw_fine = torch.cat([color, sigma], dim=2)
